...
from layouts.info_modals_layout import generate_info_modal
...
if not os.path.isdir(f"./data"):
    ...
    url = f"https://uhslc.soest.hawaii.edu/dash-data/{tarball}"
    tbfn = os.path.basename(url)
    with urllib.request.urlopen(url) as response, open(tbfn, "wb") as out_file:
        shutil.copyfileobj(response, out_file)

    with tarfile.open(tarball, "r") as tb:
        tb.extractall(".")

    os.rename(tarball[:-7], "data")
...
info_modals = [generate_info_modal(e) for e in modal_elements]
...
@app.callback(
    ...
    Output("source-station-string", "children"),
    Output("source-station-link", "href"),
    ...
    *[Output(pg, "className") for pg in about_sections],
    ...
    Output("threshold-picker-modal-source", "children"),
    Output("threshold-picker-modal-source-link", "href"),
    Output("threshold-picker-modal-method", "children"),
...
def update_page(
    ...
    noaa_id = stations.loc[station_id].noaa_id

    source_station_string = (
        f"NOAA CO-OPS Station {noaa_id}"
        if noaa_id is not None
        else f"UH Sea Level Center Station {station_id}"
    )

    source_station_link = (
        f"https://co-ops.nos.noaa.gov/stationhome.html?id={noaa_id}"
        if noaa_id is not None
        else f"https://uhslc.soest.hawaii.edu/stations/?stn={station_id}"
    )

    thresholds_source = (
        "CO-OPS API For Data Retrieval"
        if noaa_id is not None
        else "UH Sea Level Center"
    )

    thresholds_source_link = (
        "https://api.tidesandcurrents.noaa.gov/api/prod/"
        if noaa_id is not None
        else "https://uhslc.soest.hawaii.edu"
    )

    thresholds_method = (
        "" if noaa_id is not None else "an extreme value analysis performed by "
    )

    # update threshold options
    if station_id != current_station_id or trigger == "units-toggle.value":
...
def update_slr_graph(
    ...
    trigger = dash.callback_context.triggered[0]["prop_id"]
    relevant_triggers = ["scenario-select.value", "units-toggle.value"]
    ...
    if trigger in relevant_triggers:
        ...
        # data = [d for d in slr_graph.data if d.name == "Observed"][0]

        # print(" ")
        # print(station_id_store)
        # print([(yr, val) for yr, val in zip(data.x, data.y)])
...
def update_slr_budget_graph(
    ...
    budget_year = (
        2020 if budget_year < 2020 else (2100 if budget_year > 2100 else budget_year)
    )
...
@app.callback(
    [Output(f"{e}-info-modal", "is_open") for e in modal_elements],
    [Input(f"{e}-info-button", "n_clicks") for e in modal_elements],
    [State(f"{e}-info-modal", "is_open") for e in modal_elements],
)
def toggle_modal(*inputs):
    ...
    if elem in modal_elements:

        ne = modal_elements.index(elem)
        n_clicks = inputs[ne]
        is_open = [
            not io if (n == ne) and n_clicks else io
            for n, io in enumerate(inputs[len(modal_elements) :])
        ]
