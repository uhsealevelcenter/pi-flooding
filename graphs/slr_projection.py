...
def slr_projection(slr, scn_focus="int", units="ft"):
    ...
    # years, values = [[]], [[]]
    # for y, v in zip(slr["observations"]["years"], slr["observations"]["values"]):
    #     if np.isnan(v) and len(values[-1]) > 0:
    #         years.append([])
    #         values.append([])
    #     elif ~np.isnan(v):
    #         years[-1].append(y)
    #         values[-1].append(round(v * uf, 3))

    # for k, (yrs, vals) in enumerate(zip(years, values)):
    #     print(vals)
    #     fig.add_trace(
    #         {
    #             "x": yrs,
    ...
    #             "type": "scatter",
    #             "mode": "lines+markers",
    #             "name": "Observed" if k == 0 else f"Observed {k}",
